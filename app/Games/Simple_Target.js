define(['jquery', 'matter-js', 'pixi', 'games/CommonGameMixin', 'howler'], function($, Matter, PIXI, CommonGameMixin, h) {
	...
	var game = {
			...
			this.addEventListener('mousedown', function(event) { 
				...
				var y = event.data.global.y;
					...
					if(Matter.Vertices.contains(this.ball.verticeCopy, {x: x, y: y})) {
						...
						this.getAnimation('blueCollapse', [this.ball.positionCopy.x, this.ball.positionCopy.y, (this.ball.circleRadius*2/512), (this.ball.circleRadius*2/512)], .6).play();
		...
		createBallCenter: function(ball) {
			...
			this.ballTarget.render.sprite.texture = this.texture('bluetarget2Center');
