...
var equipShow = gameUtils.getSound('menuopen1.wav', {
    volume: 0.08,
    rate: 1.0
});
var equipHide = gameUtils.getSound('menuopen1.wav', {
    volume: 0.05,
    rate: 1.25
});
var equip = gameUtils.getSound('augmentequip2.wav', {
    volume: 0.18,
    rate: 1.20
});
var unequip = gameUtils.getSound('augmentEquip.wav', {
    volume: 0.02,
    rate: 0.85
});
var hoverAugmentSound = gameUtils.getSound('augmenthover.wav', {
    volume: 0.015,
    rate: 1
});
var unlockAugmentSound = gameUtils.getSound('unlockability.wav', {
    volume: 0.16,
    rate: 1
});
...
ConfigPanel.prototype.initialize = function() {
    ...
    this.showButton = graphicsUtils.createDisplayObject('AugmentNotificationPanelBorder', {
        where: 'hudNOne',
        position: {
            x: this.unitPanelRef.abilityCenterX,
            y: gameUtils.getPlayableHeight() + this.configButtonHeight / 3
        }
    });
    this.showButton.interactive = true;
    this.showButtonGlass = graphicsUtils.createDisplayObject('AugmentNotificationPanelGlass', {
        where: 'hudNOne',
        position: {
            x: this.unitPanelRef.abilityCenterX,
            y: gameUtils.getPlayableHeight() + this.configButtonGlassHeight / 3
        }
    });
    graphicsUtils.graduallyTint(this.showButtonGlass, 0x62f6db, 0xd1b877, 15000, null, 5000);

    var abilityMin = {
        x: this.unitPanelRef.abilityCenterX - 122,
        y: gameUtils.getPlayableHeight() - 40
    };
    var abilityMax = {
        x: this.unitPanelRef.abilityCenterX + 122,
...
ConfigPanel.prototype.flashPanel = function(unit) {
    ...
    this.flashTimer = graphicsUtils.flashSprite({
        sprite: this.showButton,
        duration: 150,
        ...
        if (ability.augments) {
            ...
            ability.getAvailableAugments().forEach((augment, j) => {
                ...
                if (!augment.icon.parent) {
                    var a = graphicsUtils.addSomethingToRenderer(augment.icon, {
                        position: {
                            x: ability.icon.position.x,
                            y: gameUtils.getPlayableHeight() + this.initialYOffset + this.spacing * (j)
                        },
                        where: 'hudOne'
                    });
                    // augment.lock.visible = false;
                    // augment.unlocked = true; //for debugging
                    var ab = augment.actionBox = graphicsUtils.addSomethingToRenderer('TransparentSquare', {
                        position: {
                            x: ability.icon.position.x,
                            y: gameUtils.getPlayableHeight() + this.initialYOffset + this.spacing * (j)
                        },
                    ...
                    graphicsUtils.makeSpriteSize(augment.actionBox, {
                        x: 50,
                        y: 50
                    });
                    var b = augment.border = graphicsUtils.addSomethingToRenderer('AugmentBorder', {
                        position: {
                            x: ability.icon.position.x,
                            y: gameUtils.getPlayableHeight() + this.initialYOffset + this.spacing * (j)
                        },
                        ...
                        if (globals.currentGame.itemSystem.isGrabbing()) {
                            ...
                            if (augment.currentMicrochip) {
                                ItemUtils.createItemAndGrasp({
                                    gamePrefix: "Us",
                                    itemName: [augment.currentMicrochipName],
                                    unit: this.prevailingUnit
                                });
                            ...
                            Tooltip.makeTooltippable(augment.actionBox, {
                                title: augment.title,
                                description: augment.description,
                                updaters: augment.updaters,
                                systemMessage: finalSysMessage
                            });
                            ...
                            Matter.Events.trigger(globals.currentGame.unitSystem, 'augmentEquip', {
                                augment: augment,
                                unit: this.prevailingUnit
                            });
