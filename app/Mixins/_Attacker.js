define(['jquery', 'matter-js', 'pixi', 'games/CommonGameMixin'], function($, Matter, PIXI, CommonGameMixin) {
    ...
    return {
        ...
        _initAttacker: function() {
		    ...
		    this.cooldownTimer = currentGame.addTimer({name: 'cooldown' + this.body.id, runs: 0, timeLimit: this.cooldown, callback: function() {
                this.canAttack = true;
		    }.bind(this)});
        ...
        _attack: function(target) {
            if(this.canAttack) {
                ...
                if(this.attack) {
                    this.attack(target);
					Matter.Events.trigger(this, 'attack', {direction: currentGame.isoDirectionBetweenPositions(this.position, target.position)});
                }
