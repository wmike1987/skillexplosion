...
var initiateBlinkDeath = function(options) {
    ...
    var t = globals.currentGame.addTimer({
        name: 'itemRemove' + item.body.id,
        runs: timerTime,
        timeLimit: 50,
        callback: function() {
...
var getRandomItemsFromClass = function(options) {
    var itemClass = options.itemClass;
    var itemType = options.itemType;
    var amount = options.amount;
    var uniqueItem = options.uniqueItem;
    ...
    do {
        ...
        if (!returnItems.includes(chosenItem) && !(uniqueItem && isItemInPlayByPlayer(chosenItem))) {
            collision = false;
...
var resolveItemInformation = function(options) {
    ...
    return {
        itemName: chosenItem,
        classInformation: {
            itemClassContext: ItemClasses[chosenClassKey],
            itemType: chosenTypeKey,
            itemClass: chosenClassKey,
            typeInfo: ItemClasses[chosenClassKey][chosenTypeKey]
        }
    };
};
...
export default {
    initiateBlinkDeath: initiateBlinkDeath,
    giveUnitItem: giveUnitItem,
    dropItemAtPosition: dropItemAtPosition,
    createItemObj: createItemObj,
    createItemAndGrasp: createItemAndGrasp,
    getRandomItemsFromClass: getRandomItemsFromClass,
    getRandomItemFromClass: getRandomItemFromClass,
    isItemInPlayByPlayer: isItemInPlayByPlayer
};
