...
var cantpickup = gameUtils.getSound('cantpickup.wav', {
    volume: 0.03,
    rate: 1.3
});
var itemGrab = gameUtils.getSound('itemgrab.wav', {
    volume: 0.045,
    rate: 1
});
...
var itemSystem = function(properties) {
    ...
    this.initialize = function(options) {
            ...
            if (item) {
                ...
                if (!pickedUp) {
                    if (item.owningUnit.isDead) {
                        ...
                        var dropPosition = Matter.Vector.add(item.owningUnit.position, {
                            x: 35 - variationX,
                            y: 35
                        });
                        do {
        ...
        Matter.Events.on(this, "usergrab", function(event) {
            ...
            this.canDropGrabbedItem = event.autoGrab;
            itemGrab.play();
        ...
        this.grabDropListener = globals.currentGame.addListener('mousedown', function(event) {
            ...
            if (gameUtils.isPositionWithinPlayableBounds(globals.currentGame.mousePosition)) {
                ...
                if (item.dropCallback) {
                    dropAnyway = item.dropCallback(globals.currentGame.mousePosition);
                }
                    ...
                    if (loopItem && loopItem != item && !loopItem.isEmptySlot) {
                        if (loopItem.icon.containsPoint(globals.currentGame.renderer.interaction.mouse.global)) {
                            if (item.worksWithSlot(loopItem.currentSlot)) {
                                ...
                                Matter.Events.trigger(this, 'usergrab', {
                                    item: loopItem
                                });
                ...
                if (!found) {
                    if (!prevailingUnit.pickupItem(item, item.currentSlot)) {
                        let variationX = Math.random() * 60;
                        let variationY = Math.random() * 60;
                        item.drop(Matter.Vector.add(item.owningUnit.position, {
                            x: 35 - variationX,
                            y: 35 - variationY
                        }));
                        item.icon.visible = false;
